...
ACTOR_BINARY="CUDA_VISIBLE_DEVICES='' python3 ../${ENVIRONMENT}/${AGENT}_main.py --run_mode=actor --doom_env=doom_health_gathering_supreme --learning_rate=0.0001 --inference_batch_size=10 --entropy_cost=0.003";
NON_ACTOR_BINARY="python3 ../${ENVIRONMENT}/${AGENT}_main.py --run_mode=actor --doom_env=doom_health_gathering_supreme --learning_rate=0.0001 --inference_batch_size=10 --entropy_cost=0.003";
LEARNER_BINARY="python3 ../${ENVIRONMENT}/${AGENT}_main.py --run_mode=learner --doom_env=doom_health_gathering_supreme --learning_rate=0.0001 --inference_batch_size=10 --entropy_cost=0.003";
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
...
for ((id=0; id<$TMUX_SESSIONS; id++)); do
    ...
    COMMAND=''"${ACTOR_BINARY}"' --logtostderr --pdb_post_mortem '"$@"' --doom_env=doom_health_gathering_supreme --learning_rate=0.0001 --inference_batch_size=10 --entropy_cost=0.003 --num_actors='"${WORKERS}"' --task='"${id}"''
