...
if [[ "19.03" > $docker_version ]]; then
  ...
  docker run --ulimit nofile=5000 --oom-kill-disable  --gpus '"device=0"' -p 0.0.0.0:6009:6009 --entrypoint ./docker/run.sh -ti -it -e HOST_PERMS="$(id -u):$(id -g)" --name seed --rm seed_rl:$ENVIRONMENT $ENVIRONMENT $AGENT $NUM_ACTORS
fi
