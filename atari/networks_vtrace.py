...
class AtariConvnet(tf.Module):
  def __init__(self, num_actions):
    ...
    self._stacks = [
      tf.keras.layers.Conv2D(32, [8, 8], 4, padding='same', activation='relu'),
      tf.keras.layers.Conv2D(64, [4, 4], 2, padding='same', activation='relu'),
      tf.keras.layers.Conv2D(128, [3, 3], 2, padding='same', activation='relu'),
    ]
